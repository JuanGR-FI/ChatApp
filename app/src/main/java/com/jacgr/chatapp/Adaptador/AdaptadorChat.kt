...
class AdaptadorChat(contexto: Context, chatLista: List<Chat>, imagenUrl: String): RecyclerView.Adapter<AdaptadorChat.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, position: Int): ViewHolder {
        return if(position == 1){
            val view = LayoutInflater.from(contexto).inflate(com.jacgr.chatapp.R.layout.item_mensaje_derecho, parent, false)
            ViewHolder(view)
        } else {
            val view = LayoutInflater.from(contexto).inflate(com.jacgr.chatapp.R.layout.item_mensaje_izquierdo, parent, false)
            ViewHolder(view)
        }
    }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if(position == chatLista.size-1){
            if(chat.isVisto()){
                ...
                if(chat.getMensaje().equals("Se ha enviado una imagen") && !chat.getUrl().equals("")){
